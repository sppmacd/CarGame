...
{
	// Background 1
	RectangleShape rsx(Vector2f(1920,1080));
	...
	// Text
	Text tx = GameDisplay::instance->drawCenteredString(vstr, 30, Vector2f(wnd->getSize().x / 2, 300));
	...
	rsb.setPosition(wnd->getSize().x / 2.f, 340);
...
void GuiYesNo::onLoad()
{
	...
	tx.setPosition(wnd->getSize().x / 2, 300);

	RectangleShape rsb(Vector2f(tx.getLocalBounds().width + 40.f, tx.getLocalBounds().height + 90.f));
	rsb.setOrigin(rsb.getSize() / 2.f);
	rsb.setFillColor(Color(110, 100, 65));
	rsb.setPosition(Vector2f(wnd->getSize()) / 2.f);
